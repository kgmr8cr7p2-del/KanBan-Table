...
import { CheckCheck, FileText, Paperclip, Send, X } from "lucide-react";
    ...
    <aside className="direct-chat-backdrop" role="dialog" aria-modal="true" aria-labelledby="direct-chat-title" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="direct-chat-panel">
        <header className="direct-chat-head">
          ...
          <div>
            <h2 id="direct-chat-title">{user.name}</h2>
            <span className={`direct-chat-presence ${presenceTone(user)}`}><i aria-hidden="true" />{status}</span>
          </div>
          <button className="button icon secondary" type="button" aria-label="Закрыть чат" onClick={onClose}><X size={18} /></button>
        </header>

        <div className="direct-chat-messages" ref={listRef} aria-live="polite">
