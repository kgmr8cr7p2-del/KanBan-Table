...
export async function PATCH(request: Request, { params }: Params) {
  try {
    ...
    if (id === actor.id && (input.approved === false || (nextRole && !roleHasPermission(nextRole, PermissionKey.MANAGE_USERS)))) {
    ...
    const removesAdminAccess = roleHasPermission(target.role, PermissionKey.MANAGE_USERS)
      && (input.approved === false || (nextRole !== null && !roleHasPermission(nextRole, PermissionKey.MANAGE_USERS)));
